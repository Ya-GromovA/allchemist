        ...
        with (
            ...
            pass_login = self.client.post(
                ...
                json={
                    ...
                    "password": "Valid-Synthetic-Test-Password-001",
                },
            )
            ...
            pass_login_bad = self.client.post(
                "/api/v1/admin/auth/login-password",
                json={"login": "auth-contract-test-admin", "password": "wrong"},
            )
            self.assertEqual(pass_login_bad.status_code, 401, pass_login_bad.text)
            self.assertEqual(
                pass_login_bad.json(),
